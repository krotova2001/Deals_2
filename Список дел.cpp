...
deal* Add(deal* m, int &s, deal ni) // функция добавления одного дела в конец с массивом дел. Принимает массив дел, размер и новое дело
{
    deal* n = new deal[s + 1]; // создаем новый массив на 1 больше чем был
    for (int i = 0; i < s; i++) //в цикле копируем все предыдщее содержание массива в новый массив
    {
        n[i].date = m[i].date;
        n[i].id = m[i].id;
    ...
    //и списком копируем новое дело в свободное место - конец нового массива
    n[s].date = ni.date;
    n[s].id = ni.id;
...
void Write_deals(deal* a, int size) // функция записи базы данных дел в файл
{
    ...
    {
        fwrite(a, sizeof(deal), size, file); //записываем целиком все структуры за раз
...
{
    if (a != nullptr) // проверяем свободен ли указатель
        delete[]a; // если нет - очищаем
    a = nullptr;
    s = 0; // обнуляем размер 
    FILE* pfile = nullptr; 
    ...
    if (pfile != nullptr)
    {
        deal ni; // создаем новое пустое дело
...
int main()
{
    ...
    Deals_all = Load_deal(Deals_all, size); // сразу при загрузке программы загражем БД
    int choise;
    do {
